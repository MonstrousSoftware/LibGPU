...
public class Device implements Disposable {
    ...
    public Device(Adapter adapter) {
        ...
        if(LibGPU.app.configuration.enableGPUtiming){
...
//        System.out.println("maxTextureArrayLayers " + supportedLimits.getLimits().getMaxTextureArrayLayers());
        ...
        if (LibGPU.app.configuration.enableGPUtiming && !webGPU.wgpuDeviceHasFeature(device, WGPUFeatureName.TimestampQuery)) {
            System.out.println("** Requested timestamp queries are not supported!");
        }
    }

    private Pointer getDeviceSync(Adapter adapter, WGPUDeviceDescriptor deviceDescriptor){
